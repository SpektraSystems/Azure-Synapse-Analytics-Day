...
1. From the **Language** drop-down list select **PySpark (python)** so the notebook can run by selecting both values.

    ![](../media/pyspark-lan.png)

    >**Note**: **About Synapse Notebooks & Spark Sessions**

    > Azure Synapse notebooks run code using an attached Apache Spark Pool, which spins up a temporary **"Spark Session"** to store your variables and data in memory. To prevent unnecessary cloud costs, this session will automatically shut down **(Auto-Pause) after 15 to 30 minutes of inactivity**. If you return to your notebook after a break and encounter an error, it simply means your session timed out; **running your cells again from the top will automatically initialize a new session and restore your data**.

11. Paste the given code into the new cell and **replace** `YOUR_DATALAKE_NAME` with your storage account name **<inject key="Storage Account Name"></inject> (1)**.
...
20. Next, select **Data** from the left-hand menu to view the workspace data assets.
...
2. In the Integrate menu, expand **Pipelines (1)**, then select **Exercise 2 - Enrich Data (2)**.It opens a **pipeline canvas (3)** on right side.
   ...
   > **Note**: Click **<<** to collapse the pane and view the full canvas.

   > ![](../media/arrow-expand-canvas.png)
...
4. From the canvas graph, select the **ForEach (1)** activity named `ForEachComment` and switch to the **Settings (2)** tab. The **Items** property is set to receive the output of the previous `ReadComments` **(3)** activity. You might have noticed the green connection line between the two activities that define a dependency between the two activities. The line makes sure comments are read before the ForEach loop can iterate it. In the **Activities** box, click the **pencil icon (4)** to open the loop.
...
5. Select **Copy data** activity named `Sentiment Analysis` **(1)** and switch to the **Source (2)** tab. The Copy Data activity's Source dataset is set to a **cognitive_rest_dataset (3)** resource backed by Azure Cognitive Services. A **POST (4)** HTTP request method will be made to the Azure Cognitive Services endpoint carrying a **request body (5)** that includes the text from the current iteration that will be analyzed for sentiments.
...
10. Select the **Mapping (1)** tab. On this tab, you can review and set the column mappings. As you can see on this tab at **Sources (2)** the column names have spaces, while the spaces are being removed from the **Destination (3)** column names.
...
17. To better understand how a data flow functions, let us inspect the various components. Select the **PostalCodes (1)** data source on the data flow canvas.
    ...
    > Drag the properties pane upwards **(2)**. On the **Source settings (3)** tab, we see properties similar to what we saw on the pipeline activities property tabs. The name of the component can be defined, along with the source dataset and a few other properties. The `PostalCodes` dataset points to a CSV file **external_postalcode_adls (4)** stored in an Azure Data Lake Storage Gen2 account.

    ![The PostalCodes data source component is highlighted on the data flow canvas surface.](../media/postalcodes.png "Data flow canvas")
...
33. In the data flow canvas graph, select the green On Success box on the right-hand side of the `Import Customer dimension` **(1)** Copy data activity and drag the resulting arrow up onto the `Enrich Customer Data` **(2)** Data Flow activity.
...
34. This creates a requirement that the **Copy data** activity completes successfully before the **Data Flow** can execute, and enforces our requirement of the Synapse Analytics table being populated before running the data flow.
    ...
    ![The dependency arrow going from the Copy data activity to the Mapping Data Flow is displayed.](../media/connect-arrow.png "Data pipeline canvas")
